...
def layernorm_backward(dout, cache):
    ...
    dmean = np.sum(-dx_hat / np.sqrt(var + eps), axis=1, keepdims=True) + dvar * np.mean(-2.0 * (x-mean), axis=1, keepdims=True)
      ...
      for f in range(F):
        for i_out in range(H_out):
          for j_out in range(W_out):
            ...
            for c in range(C):
              dw[f, c] += x_pad[n, c, i_in:i_in + HH,
                              j_in:j_in + WW] * dout[n, f, i_out, j_out]

              dx_pad[n, c, i_in:i_in + HH, j_in:j_in + WW] += w[f, c] * dout[n, f, i_out, j_out]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_hat_group = (x_group - sample_mean) / np.sqrt(sample_var + eps)
    x_hat = x_hat_group.reshape(N, C, H, W)

    out = gamma * x_hat + beta
    
    cache = (G, x, x_hat, sample_mean, sample_var, gamma, beta, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dmean = (
        np.sum(-dx_hat / np.sqrt(var + eps), axis=(2, 3, 4), keepdims=True)
        + dvar * np.sum(-2.0 * (x - mean), axis=(2, 3, 4), keepdims=True) / D
    )
