...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        hidden_i = affine_forward(features, W_proj, b_proj)
        x = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == "rnn":
          h = rnn_forward(x, hidden_i, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        next_word_idx = self._start * torch.ones((N,), dtype=torch.long)

        for t in range(max_length):  
          if self.cell_type == "rnn":
            embed = word_embedding_forward(next_word_idx, W_embed)
            h = rnn_step_forward(embed, h, Wx, Wh, b)
            scores = affine_forward(h, W_vocab, b_vocab)
            next_word_idx = torch.argmax(scores, dim=1)
