...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        top_indices = torch.flatten(top_indices)
...
class DINOSegmentation:
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for i in range(num_iters):
          ...
          Y_pred_logits = self.model(X_train)
          loss = self.loss_fn(Y_pred_logits, Y_train)
          loss.backward()
          self.optimizer.step()
    ...
    def inference(self, X_test):
        ...
        self.model.eval()
        Y_pred_logits = self.model(X_test)
        pred_classes = torch.argmax(Y_pred_logits, dim=1)
