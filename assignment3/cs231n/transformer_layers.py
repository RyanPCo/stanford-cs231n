...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # Transformer_Cptioning.ipynb.  The goal is for each row to alternate     #
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key_t = key.transpose(2, 3) # N, H, E/H, T

        attn = torch.matmul(query, key_t) / math.sqrt(self.head_dim) # N, H, S, T
        ...
        scores = torch.matmul(attn, value) # N, H, S, E/H
        scores = scores.transpose(1, 2).contiguous() # N, S, H, E/H
        output = scores.view(N, S, E)
